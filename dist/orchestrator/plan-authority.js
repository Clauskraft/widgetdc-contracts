...
export const PLAN_AUTHORITY_SCHEMA_IDS = {
    ...
    PlanAuthorityEnvelopeV2: 'https://widgetdc.com/contracts/orchestrator/PlanAuthorityEnvelopeV2.json',
    PlanAuthorityAdmissionResultV2: 'https://widgetdc.com/contracts/orchestrator/PlanAuthorityAdmissionResultV2.json',
};
 ...
 */
const PLAN_AUTHORITY_SCHEMA_VERSION_V2 = 'wdc.plan_authority_envelope.v2';
const PLAN_AUTHORITY_SCOPE_SCHEMA_VERSION_V2 = 'wdc.plan_authority_scope.v2';
...
function rejectedPlanAuthorityAdmissionV2(reason) {
    return {
...
function assertPlainEnumerableDataObjectV2(value, label) {
    if (value === null ||
        typeof value !== 'object' ||
        Array.isArray(value) ||
...
function ownEnumerableDataValueV2(object, key) {
...
export function canonicalPlanAuthorityScopeHashV2(scope) {
...
function projectCanonicalPlanAuthorityPayloadV2(document) {
    ...
    if (schemaVersion !== PLAN_AUTHORITY_SCHEMA_VERSION_V2) {
        throw new TypeError(`schema_version ${schemaVersion ?? '<missing>'} does not match ${PLAN_AUTHORITY_SCHEMA_VERSION_V2}`);
    }
    return payload;
}
export function canonicalPlanAuthorityPayloadHashV2(document) {
    return contentAddressedIdentity({
...
export function hasValidCanonicalPlanAuthorityScopeHashV2(document) {
    try {
        assertPlainEnumerableDataObjectV2(document, 'plan authority V2 document');
...
export function hasValidCanonicalPlanAuthorityPayloadHashV2(document) {
    try {
        assertPlainEnumerableDataObjectV2(document, 'plan authority V2 document');
        const claimedHash = ownEnumerableDataValueV2(document, 'canonical_payload_hash');
        return typeof claimedHash === 'string' &&
            /^sha256:[0-9a-f]{64}$/.test(claimedHash) &&
            canonicalPlanAuthorityPayloadHashV2(document) === claimedHash;
    }
    catch {
        return false;
    }
}
export function evaluatePlanAuthorityAdmissionV2(input, options) {

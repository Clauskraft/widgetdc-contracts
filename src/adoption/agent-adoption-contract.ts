...
export const AgentAdoptionContract = Type.Object({
  ...
  description:
    'Canonical adoption contract (B11) — single machine-readable artefact every agent on every bridge must satisfy. ' +
...
export function computeContractHash(
  contract: Omit<AgentAdoptionContract, 'hash'> & { hash?: string },
): string {
 ...
 * separately.
 */
export function verifyContractHash(contract: AgentAdoptionContract): boolean {

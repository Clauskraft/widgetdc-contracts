...
export const SYNTHESIS_RECEIPT_SCHEMA_ID =
  'https://contracts.widgetdc.dev/execution/synthesis-receipt.schema.json' as const
 ...
 */
export function validateSynthesisReceipt(value: unknown): value is SynthesisReceipt {
  if (!Value.Check(SynthesisReceipt, value)) return false
  if (value.outcome !== 'completed') return true
  if (value.artifact_checks.length === 0) return false

  return value.artifact_checks.every(
    (check) =>
      check.observed_count >= check.required_count &&
      check.schema_valid &&
